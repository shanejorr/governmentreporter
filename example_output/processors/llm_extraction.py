...
The Fourth Amendment protects against unreasonable searches and seizures. Digital devices contain vast amounts of personal information that deserve constitutional protection."""
        ...
        # Test with syllabus
        scotus_metadata = generate_scotus_llm_fields(sample_scotus_text, sample_syllabus)
        
        results["generate_scotus_llm_fields_with_syllabus"] = {
            "method": "generate_scotus_llm_fields() with syllabus",
            "result": {
                "plain_language_summary": scotus_metadata.get("plain_language_summary", "")[:200] + "..." if len(scotus_metadata.get("plain_language_summary", "")) > 200 else scotus_metadata.get("plain_language_summary", ""),
                ...
                "reasoning": scotus_metadata.get("reasoning", "")[:200] + "..." if len(scotus_metadata.get("reasoning", "")) > 200 else scotus_metadata.get("reasoning", ""),
                "constitution_cited": scotus_metadata.get("constitution_cited", []),
                "federal_statutes_cited": scotus_metadata.get("federal_statutes_cited", []),
                "federal_regulations_cited": scotus_metadata.get("federal_regulations_cited", []),
                "cases_cited": scotus_metadata.get("cases_cited", []),
                "topics_or_policy_areas": scotus_metadata.get("topics_or_policy_areas", [])
            }
        }
        ...
        results["generate_scotus_llm_fields_no_syllabus"] = {
            ...
            "result": {
                "plain_language_summary": scotus_metadata_no_syllabus.get("plain_language_summary", "")[:200] + "..." if len(scotus_metadata_no_syllabus.get("plain_language_summary", "")) > 200 else scotus_metadata_no_syllabus.get("plain_language_summary", ""),
                "topics_count": len(scotus_metadata_no_syllabus.get("topics_or_policy_areas", [])),
                "total_citations": len(scotus_metadata_no_syllabus.get("constitution_cited", [])) + len(scotus_metadata_no_syllabus.get("federal_statutes_cited", [])) + len(scotus_metadata_no_syllabus.get("cases_cited", []))
            }
        }
...
Sec. 3. Requirements. The Department of Transportation and the Environmental Protection Agency shall develop new guidelines for assessing climate risks in federal projects."""
        ...
        results["generate_eo_llm_fields"] = {
            ...
            "result": {
                "plain_language_summary": eo_metadata.get("plain_language_summary", "")[:200] + "..." if len(eo_metadata.get("plain_language_summary", "")) > 200 else eo_metadata.get("plain_language_summary", ""),
                ...
                "federal_regulations_cited": eo_metadata.get("federal_regulations_cited", []),
                "cases_cited": eo_metadata.get("cases_cited", []),
                "topics_or_policy_areas": eo_metadata.get("topics_or_policy_areas", []),
                "summary_starts_with_action_verb": any(eo_metadata.get("plain_language_summary", "").startswith(verb) for verb in ["Establishes", "Prohibits", "Requires", "Revokes", "Directs", "Creates", "Modifies", "Authorizes", "Mandates", "Rescinds"])
            }
        }
