...
class TestCourtListenerClient:
    ...
    @pytest.fixture
    def mock_docket_data(self):
        """
        Provide sample docket data from Court Listener API.

        Dockets contain court identification and case metadata that link
        to clusters and opinions.

        Returns:
            dict: Sample docket data with court information
        """
        return {
            "id": 345678,
            "resource_uri": "/api/rest/v4/dockets/345678/",
            "absolute_url": "/docket/345678/test-case-v-united-states/",
            "court": "/api/rest/v4/courts/scotus/",
            "court_id": "scotus",
            "original_court_info": None,
            "idb_data": None,
            "clusters": ["/api/rest/v4/clusters/789012/"],
            "audio_files": [],
            "assigned_to": None,
            "referred_to": None,
            "date_created": "2023-01-10T09:00:00Z",
            "date_modified": "2023-01-10T09:00:00Z",
            "source": 1,
            "appeal_from_str": "",
            "assigned_to_str": "",
            "referred_to_str": "",
            ...
            "slug": "test-case-v-united-states",
            "docket_number": "22-123",
            "docket_number_core": "22",
            "docket_number_raw": "22-123",
    ...
    @patch("httpx.Client")
    def test_get_docket_success(self, mock_httpx_client, client, mock_docket_data):
        """
        Test successful docket retrieval.

        Verifies that docket data is correctly fetched and includes
        court_id for validation purposes.
        """
        mock_client_instance = MagicMock()
        mock_httpx_client.return_value.__enter__.return_value = mock_client_instance

        docket_response = MagicMock()
        docket_response.json.return_value = mock_docket_data
        docket_response.raise_for_status = MagicMock()

        mock_client_instance.get.return_value = docket_response

        docket_url = "/api/rest/v4/dockets/345678/"
        docket = client.get_docket(docket_url)

        assert docket["id"] == 345678
        assert docket["court_id"] == "scotus"
        assert docket["case_name"] == "Test Case v. United States"
        assert docket["docket_number"] == "22-123"
        assert len(docket["clusters"]) == 1

        # Verify API call was made with correct URL
        mock_client_instance.get.assert_called_once_with(
            docket_url, headers=client.headers
        )

    @patch("httpx.Client")
    def test_get_docket_non_scotus(self, mock_httpx_client, client):
        """
        Test docket retrieval for non-SCOTUS court.

        Verifies that docket data correctly identifies different courts
        like state courts or circuit courts.
        """
        mock_client_instance = MagicMock()
        mock_httpx_client.return_value.__enter__.return_value = mock_client_instance
        ...
        docket_response = MagicMock()
        docket_response.json.return_value = ny_docket_data
        docket_response.raise_for_status = MagicMock()

        mock_client_instance.get.return_value = docket_response

        docket_url = "/api/rest/v4/dockets/71584717/"
        docket = client.get_docket(docket_url)

        assert docket["court_id"] == "nyappdiv"
        assert docket["case_name"] == "Brooklyn Tabernacle v. Thor 180 Livingston, LLC"

    @patch("httpx.Client")
    def test_get_docket_not_found(self, mock_httpx_client, client):
        """
        Test docket retrieval with non-existent docket.

        Verifies proper error handling when docket doesn't exist.
        """
        mock_client_instance = MagicMock()
        mock_httpx_client.return_value.__enter__.return_value = mock_client_instance
        ...
        with pytest.raises(httpx.HTTPStatusError) as exc_info:
            client.get_docket("/api/rest/v4/dockets/999999999/")
