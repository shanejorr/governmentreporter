...
class TestCourtListenerClientConfiguration:
    ...
    def test_base_url_configuration(self, court_listener_client):
        ...
        # Assert
        assert court_listener_client._get_base_url() == "https://www.courtlistener.com/api/rest/v4"
...
class TestGetOpinionCluster:
    ...
    def test_get_cluster_happy_path(self, court_listener_client, cluster_payload):
        ...
        # Arrange
        cluster_url = "https://www.courtlistener.com/api/rest/v4/clusters/9506542/"
    ...
    def test_get_cluster_not_found(self, court_listener_client):
        ...
        # Arrange
        cluster_url = "https://www.courtlistener.com/api/rest/v4/clusters/999999/"
    ...
    def test_get_cluster_timeout(self, court_listener_client):
        ...
        # Arrange
        cluster_url = "https://www.courtlistener.com/api/rest/v4/clusters/123/"
        
        # Mock timeout
        respx.get(cluster_url).mock(side_effect=httpx.TimeoutException("Request timed out"))
...
class TestGetDocument:
    ...
    @respx.mock
    def test_get_document_with_cluster(self, court_listener_client, opinions_payload, cluster_payload):
        ...
        assert document.metadata["citation"] == "601 U.S. 416 (2024)"  # Built from cluster
    ...
    @respx.mock
    def test_get_document_cluster_fallback(self, court_listener_client, opinions_payload):
        ...
        assert "citation" not in document.metadata or document.metadata["citation"] is None
        # case_name is added to metadata when cluster fails
        assert document.metadata.get("case_name") == "Unknown Case" or "case_name" not in document.metadata
...
class TestGetDocumentText:
    ...
    @respx.mock
    def test_get_document_text_happy_path(self, court_listener_client, opinions_payload):
...
class TestSearchDocuments:
    ...
    def test_search_documents_basic(self, court_listener_client):
        ...
        # Arrange
        search_url = "https://www.courtlistener.com/api/rest/v4/opinions/"
        search_results = {
            "count": 2,
            "next": None,
            "results": [
    ...
    def test_search_documents_with_pagination(self, court_listener_client):
        ...
        page1_results = {
            ...
            "results": [
                {"id": i, "snippet": f"Opinion {i}", "date_created": "2024-01-01T00:00:00Z"}
    ...
    def test_search_documents_with_date_filters(self, court_listener_client):
        ...
        route = respx.get(search_url).mock(return_value=Response(200, json=empty_results))
    ...
    @respx.mock
    def test_search_documents_with_full_content(self, court_listener_client, opinions_payload, cluster_payload):
        ...
        opinion_url = "https://www.courtlistener.com/api/rest/v4/opinions/9973155/"
        cluster_url = "https://www.courtlistener.com/api/rest/v4/clusters/9506542/"
        ...
        documents = court_listener_client.search_documents(
            query="test",
            limit=10,
            full_content=True  # Request full content
        )
    ...
    def test_search_documents_error_handling(self, court_listener_client):
        ...
        search_results = {
            ...
            "next": None,
            "results": [
                {"id": 111, "snippet": "Good 1", "date_created": "2024-01-01T00:00:00Z"},
                {"id": None, "snippet": "Bad - no ID"},  # Invalid - no ID
                {"id": 333, "snippet": "Good 2", "date_created": "2024-01-03T00:00:00Z"}
            ]
        }
...
class TestDateValidation:
    ...
    def test_validate_date_format_invalid(self, court_listener_client):
        ...
        assert court_listener_client.validate_date_format("2024-1-15") is False  # Missing zeros
    ...
    def test_validate_date_format_edge_cases(self, court_listener_client):
        ...
        assert court_listener_client.validate_date_format("2024-13-45") is True  # Invalid but correct format
        assert court_listener_client.validate_date_format("2024-00-00") is True  # Invalid but correct format