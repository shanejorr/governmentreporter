...
class TestServerCommand:
    ...
    def test_server_starts_with_valid_config(self, mock_getenv, mock_run_server, cli_runner):
    ...
    def test_server_validates_openai_key(self, mock_getenv, cli_runner):
        ...
        if result.exit_code != 0:
            assert "OPENAI" in result.output or "API key" in result.output or "error" in result.output.lower()

    @patch('governmentreporter.cli.server.create_and_run_server')
    @patch('governmentreporter.cli.server.os.getenv')
    def test_server_handles_startup_errors(self, mock_getenv, mock_run_server, cli_runner):
    ...
    @patch('governmentreporter.cli.server.create_and_run_server')
    @patch('governmentreporter.cli.server.os.getenv')
    def test_server_respects_environment_variables(self, mock_getenv, mock_run_server, cli_runner):
...
class TestServerCommandIntegration:
    ...
    @patch('governmentreporter.cli.server.create_and_run_server')
    @patch('governmentreporter.cli.server.os.getenv')
    def test_server_logs_startup_message(self, mock_getenv, mock_run_server, cli_runner):
    ...
    @patch('governmentreporter.cli.server.create_and_run_server')
    @patch('governmentreporter.cli.server.os.getenv')
    def test_server_can_be_invoked_multiple_times(self, mock_getenv, mock_run_server, cli_runner):
