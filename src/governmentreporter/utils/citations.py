...
def parse_usc_citations(text: str) -> list[Dict[str, str]]:
    ...
    pattern = r'(\d{1,2})\s*U\.?\s*S\.?\s*C\.?\s*(?:§§?\s*)?(\d+(?:\.\d+)?(?:\([a-z0-9]+\))*)'
...
def parse_constitution_citations(text: str) -> list[Dict[str, Optional[str]]]:
    ...
    amendment_pattern = r'(?:U\.?S\.?\s*Const\.?\s*)?[Aa]mend(?:ment)?\s+([IVX]+)(?:,?\s*§\s*(\d+))?'
    ...
    ordinal_map = {
        ...
        'fifteenth': 'XV', 'sixteenth': 'XVI', 'seventeenth': 'XVII', 'eighteenth': 'XVIII',
        'nineteenth': 'XIX', 'twentieth': 'XX', 'twenty-first': 'XXI', 'twenty-second': 'XXII',
        'twenty-third': 'XXIII', 'twenty-fourth': 'XXIV', 'twenty-fifth': 'XXV',
        'twenty-sixth': 'XXVI', 'twenty-seventh': 'XXVII'
    }
